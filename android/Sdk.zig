...
pub fn toolchainHostTag() []const u8 {
    ...
    return std.fmt.comptimePrint("{s}-{s}", .{ os_name, arch_name });
}
...
pub const CreateAppStep = struct {
    ...
    // Reference to the WriteToZip step for adding additional native libraries
    copy_to_zip_step: *WriteToZip,
    align_step: *std.Build.Step.Run,

    pub fn getAndroidPackage(self: @This(), name: []const u8) std.Build.Module {
        return self.sdk.b.dupePkg(std.Build.Module{
            .name = name,
            .root_source_file = self.sdk.b.path(sdkRoot() ++ "/src/android-support.zig"),
            .imports = &[_]std.Build.Module{
...
pub fn createApp(
    ...
    const write_files = sdk.b.addWriteFiles();

    const strings_xml = write_files.add("strings.xml", blk: {
        var buf: std.ArrayList(u8) = .{};
        errdefer buf.deinit(sdk.b.allocator);

        const writer = buf.writer(sdk.b.allocator);
    ...
    const manifest_xml = write_files.add("AndroidManifest.xml", blk: {
        var buf: std.ArrayList(u8) = .{};
        errdefer buf.deinit(sdk.b.allocator);
    ...
    make_unsigned_apk.addFileArg(manifest_xml);
    ...
    const copy_to_zip_step_ptr = sdk.b.allocator.create(WriteToZip) catch unreachable;
    copy_to_zip_step_ptr.* = WriteToZip.init(sdk, unaligned_apk_file, unaligned_apk_name);
    copy_to_zip_step_ptr.run_step.step.dependOn(&make_unsigned_apk.step);

    var libs: std.ArrayList(*std.Build.Step.Compile) = .{};
    ...
    const android_module = sdk.b.addModule("android", .{
        .root_source_file = sdk.b.path("src/android-support.zig"),
        .imports = &.{.{
            .name = "build_options",
    ...
    align_step.addFileArg(copy_to_zip_step_ptr.output_source);
    ...
    if (java_files_opt) |java_files| {
        ...
        for (java_files) |java_file| {
            ...
            d8_cmd_builder.addFileArg(.{ .cwd_relative = class_file });
        ...
        copy_to_zip_step_ptr.addFile(.{ .cwd_relative = dex_file }, "classes.dex");
        copy_to_zip_step_ptr.run_step.step.dependOn(&make_unsigned_apk.step); // enforces creation of APK before the execution
        align_step.step.dependOn(&copy_to_zip_step_ptr.run_step.step);
    }
    ...
    inline for (std.meta.fields(AppTargetConfig)) |fld| {
        ...
        if (@field(targets, fld.name).?) {
            ...
            copy_to_zip_step_ptr.addFile(step.getEmittedBin(), target_filename);
            copy_to_zip_step_ptr.run_step.step.dependOn(&step.step);
            align_step.step.dependOn(&copy_to_zip_step_ptr.run_step.step);
        }
    }
    ...
    return CreateAppStep{
        ...
        .copy_to_zip_step = copy_to_zip_step_ptr,
...
pub fn compileAppLibrary(
    ...
    const resolved_target = sdk.b.resolveTargetQuery(config.target);
    const lib_module = sdk.b.createModule(.{
    ...
    // PIC is automatically enabled for shared libraries in Zig 0.15.1
    ...
    const libc_file = sdk.createLibCFile(app_config.target_version, config.out_dir, include_dir, system_include_dir, lib_dir) catch unreachable;
    exe.setLibCFile(libc_file);
    libc_file.addStepDependencies(&exe.step);
...
fn createLibCFile(sdk: *const Sdk, version: AndroidVersion, folder_name: []const u8, include_dir: []const u8, sys_include_dir: []const u8, crt_dir: []const u8) !std.Build.LazyPath {
    ...
    const writer = contents.writer(sdk.b.allocator);
    ...
    const write_files = sdk.b.addWriteFiles();
    return write_files.add(fname, contents.items);
}
...
// Note: OpenXR loader is provided by Quest runtime, not linked at build time
...
const BuildOptionStep = struct {
    ...
    pub fn getPackage(self: *Self, name: []const u8) std.Build.Module {
        return self.builder.dupePkg(std.Build.Module{
            .name = name,
    ...
    pub fn add(self: *Self, comptime T: type, name: []const u8, value: T) void {
        ...
        switch (T) {
            []const []const u8 => {
                out.print("pub const {s}: []const []const u8 = &[_][]const u8{{\n", .{name}) catch unreachable;
                for (value) |slice| {
            ...
            [:0]const u8 => {
                out.print("pub const {s}: [:0]const u8 = \"{s}\";\n", .{ name, value }) catch unreachable;
                return;
            },
            []const u8 => {
                out.print("pub const {s}: []const u8 = \"{s}\";\n", .{ name, value }) catch unreachable;
                return;
            },
            ?[:0]const u8 => {
                out.print("pub const {s}: ?[:0]const u8 = ", .{name}) catch unreachable;
                if (value) |payload| {
            ...
            ?[]const u8 => {
                out.print("pub const {s}: ?[]const u8 = ", .{name}) catch unreachable;
                if (value) |payload| {
                    out.print("\"{s}\";\n", .{payload}) catch unreachable;
                } else {
                    out.writeAll("null;\n") catch unreachable;
            ...
            // std.builtin.Version was removed in Zig 0.15.1, use SemanticVersion instead
            std.SemanticVersion => {
                out.print(
                    \\pub const {s}: @import("std").SemanticVersion = .{{
                    \\    .major = {d},
                    \\    .minor = {d},
                    \\    .patch = {d},
                    \\
...
const CacheBuilder = struct {
    ...
    fn createPath(self: *Self) ![]const u8 {
        ...
        // Format hash as hex string manually
        var hex_buf: [40]u8 = undefined;
        const hex_str = std.fmt.bytesToHex(&hash, .lower);
        @memcpy(hex_buf[0..hex_str.len], &hex_str);

// Shared VR application logic for teleportation locomotion
...
pub const VRApp = struct {
    ...
    pub fn update(self: *VRApp) void {
        ...
        if (self.has_vr and rl.getActionBooleanClicked(self.bindings.hand_teleport_action, self.bindings.hand_sub_paths[0])) {
            // Calculate world space hand position
            const world_hand_position = c.Vector3Add(self.left_hand.position, self.stage_position);

            // Calculate teleport destination
            const t = calculateParabolaTimeToFloor(world_hand_position, self.left_hand.orientation);
            self.stage_position = sampleParabolaPosition(world_hand_position, self.left_hand.orientation, t);
        }
    }
    ...
    pub fn render(self: *VRApp) void {
        ...
        const world_camera = c.Camera3D{
            .position = c.Vector3Add(self.camera.position, self.stage_position),
            .target = c.Vector3Add(self.camera.target, self.stage_position),
            .up = self.camera.up,
            .fovy = self.camera.fovy,
            .projection = self.camera.projection,
        };

        const left_world_pos = c.Vector3Add(self.left_hand.position, self.stage_position);
        const right_world_pos = c.Vector3Add(self.right_hand.position, self.stage_position);
        ...
        if (rendering_vr) {
            ...
            if (self.left_hand.valid) {
                ...
                c.DrawModelEx(self.hand_model, left_world_pos, axis, angle * c.RAD2DEG, c.Vector3One(), c.ORANGE);
            }
            ...
            if (self.right_hand.valid) {
                ...
                c.DrawModelEx(self.hand_model, right_world_pos, axis, angle * c.RAD2DEG, c.Vector3One(), c.PINK);
            }
            ...
            if (self.left_hand.valid) {
                ...
                while (i <= ARC_SEGMENTS) : (i += 1) {
                    ...
                    const arc_position_0 = sampleParabolaPosition(world_hand_position, self.left_hand.orientation, interpolation_t_0);
                    const arc_position_1 = sampleParabolaPosition(world_hand_position, self.left_hand.orientation, interpolation_t_1);
